...
When this Skill is loaded from a plugin bundle, `launcher.json` exists two
parent directories above the directory containing this `SKILL.md`. Read that
metadata, select only the declared Windows or Unix binary name for the current
host, and invoke the exact regular file below the bundle's `bin/` directory.
Run its declared capabilities probe before doctor and require the exact CLI,
protocol, context, next-action, standards, and capability identities from
`launcher.json`.

When no bundle `launcher.json` exists, invoke `mino` from the environment as in
the repository installation. Never modify `PATH`, download a binary, search for
an alternate executable, or execute plugin-provided code beyond the declared
Mino binary. If the bundled binary is missing, wrong-platform, or incompatible,
stop with environment-unavailable guidance; do not fall back to another Mino.
